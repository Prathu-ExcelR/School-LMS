...
import { getParentChildren, getChildAttempts } from '../../services/parentService'
...
function ParentResults() {
    ...
    useEffect(() => {
        if (selectedChild) {
            loadChildResults(selectedChild.student_id)
        }
    }, [selectedChild])
    ...
    const loadChildResults = async (studentId) => {
        try {
            const data = await getChildAttempts(studentId)
            setAttempts(data)
        } catch (err) {
            ...
            <main className="ml-64 flex-1 overflow-y-auto">
                <header className="bg-white border-b border-slate-200 px-8 py-6 flex items-center justify-between">
                    <div>
                        <nav className="flex text-sm text-slate-500 mb-2">
                            ...
                            <span className="text-slate-800 font-medium">Quiz Results</span>
                        </nav>
                        <h1 className="text-2xl font-bold text-slate-800">Child's Quiz Results</h1>
                        <p className="text-sm text-slate-500 mt-1">Review all quiz attempts and scores</p>
                    </div>
                ...
                <div className="p-8">
                    ...
                    {selectedChild?.student && (
                        <div className="bg-gradient-to-r from-[#5048e5] to-purple-600 rounded-xl p-4 text-white mb-6">
